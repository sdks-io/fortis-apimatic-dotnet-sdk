...
|  --- | --- | --- | --- |
| `ThreeDsServerTransId` | `string` | Optional | Universally unique transaction identifier assigned by the 3DS Server to identify a single transaction. It has the same value as the corresponding received authentication request. This value has 36 characters in a format defined in IETF RFC 4122. |
...
| `ErrorComponent` | `string` | Optional | Code indicating the 3-D Secure component that identified the error.<br><br>> C - 3DS SDK<br>> <br>> S - 3DS Server<br>> <br>> D - DS<br>> <br>> A - ACS |
| `ErrorDescription` | `string` | Optional | Text describing the problem identified. |
